...
export default {
  ...
  methods: {
    ...
    validateMessage(event) {
      const { target } = event
      this.validation.messageCurrent =
        this.validation.messageLimit - target.value.length
      ...
      if (target.value.length <= 0)
        this.validation.errors.message = 'Please insert a message'
      else if (target.value.length > 250)
        this.validation.errors.message = 'The length must be minor than 250'
